...
class Model:
	...
	def __init__(self, symbol, timestep, outstep, model_type, verbose=0):
	...
	def split_sequence(raw_seq, n_steps, out_steps):
		# Splits the data up into the windows specified by n
		X, y = list(), list()
		for i in range(len(raw_seq)):
			# find the end of this pattern
			end_ix = i + n_steps
			out_end_ix = end_ix + out_steps
	...
	def split_data(raw_seq, n_steps, outstep, size):

		# split into samples
		X, y = Model.split_sequence(raw_seq, n_steps, outstep)
	...
	def data_prep(self, open_col, high_col, low_col, clos_col, raw_seq):
		...
		X_train, X_test, y_train, y_test = Model.split_data(
			raw_seq, self.timestep, self.outstep, 0.2)
