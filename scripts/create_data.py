...
class make_chart_test:
    ...
    def load_data(currency_pairs):
        ...
        # Loads the CSV data
        file_loc = '/home/ryan/Documents/Python/Project-Soros/scripts/Finance_Data/Raw_Data/' + currency_pairs + '.csv'
        data = pd.read_csv(
            file_loc, index_col=0, parse_dates=True)
        # Prints the head
        print(data)
    ...
    def create(data, csv_rows, X):
        ...
        while w < n:
            ...
            mpf.plot(data[q:w], type='candle', show_nontrading=False,
                     savefig=dict(fname='/home/ryan/Documents/Python/Project-Soros/scripts/Finance_Data/Chart_Snapshot/TEST/operation/{name}_{number}test.jpg'.format(name=X, number=num)), style=s, axisoff=False)
            f = open(
                "/home/ryan/Documents/Python/Project-Soros/darknet/data/test.txt", 'a')
            f.write(
                "\n/home/ryan/Documents/Python/Project-Soros/scripts/Finance_Data/Chart_Snapshot/TEST/operation/{name}_{number}test.jpg".format(name=X, number=num))
            q += 50
            w += 50
            num += 1
            # launch.darknet(X)
